...
export default function TrustBar() {
  return (
    <section className="border-y border-primary/10 band-blue">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-5 grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-5">
        {ITEMS.map((it) => (
          <div key={it.label} className="flex items-center gap-3 justify-self-center md:justify-self-start w-[160px] md:w-auto">
            ...
            <div className="min-w-0">
              <p className="text-[13px] font-semibold text-foreground leading-tight whitespace-nowrap">{it.label}</p>
              <p className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground mt-0.5 whitespace-nowrap">{it.sub}</p>
            </div>
          </div>
        ))}
      </div>
