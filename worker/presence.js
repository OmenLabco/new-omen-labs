...
async function ensureTable(env) {
  ...
  await env.DB.prepare(
    'CREATE TABLE IF NOT EXISTS presence (sid TEXT PRIMARY KEY, page TEXT, path TEXT, state TEXT, cart_count INTEGER, updated_at INTEGER, lat REAL, lon REAL, country TEXT, city TEXT, region TEXT)'
  ).run();
  // Add geo columns to tables created before geo existed. ALTER throws
  // "duplicate column" once they exist — safe to ignore.
  for (const col of ['lat REAL', 'lon REAL', 'country TEXT', 'city TEXT', 'region TEXT']) {
...
export async function recordPresence(request, env) {
  ...
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO presence (sid, page, path, state, cart_count, updated_at, lat, lon, country, city, region) VALUES (?,?,?,?,?,?,?,?,?,?,?)
       ON CONFLICT(sid) DO UPDATE SET page=excluded.page, path=excluded.path, state=excluded.state, cart_count=excluded.cart_count, updated_at=excluded.updated_at, lat=excluded.lat, lon=excluded.lon, country=excluded.country, city=excluded.city, region=excluded.region`
    ).bind(sid, String(b.page || '').slice(0, 60), String(b.path || '').slice(0, 120), String(b.state || 'browsing').slice(0, 20), Number(b.cartCount) || 0, now, geo.lat, geo.lon, geo.country, geo.city, geo.region).run();
...
export async function liveStats(request, env) {
  ...
  if (!env.DB) return json({ online: 0, carts: 0, checkingOut: 0, itemsInCarts: 0, pages: [], sessions: [], locations: [], countries: [] });
  await ensureTable(env);
  const now = Date.now();
  const cutoff = now - 60000; // active in last 60s
  const { results } = await env.DB.prepare(
    'SELECT sid, page, path, state, cart_count, updated_at, lat, lon, country, city, region FROM presence WHERE updated_at >= ? ORDER BY updated_at DESC'
  ).bind(cutoff).all();
  const rows = results || [];
  ...
  return json({ online, carts, checkingOut, viewingProduct, itemsInCarts, pages, sessions, locations, countries });
}
